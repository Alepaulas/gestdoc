...
export async function GET(req) {
  ...
  if (!docId) return NextResponse.json({ error: "ID obrigatorio" }, { status: 400 });
  ...
  const doc = await queryOne(`
    ...
    JOIN Area a ON d.areaId=a.id JOIN Setor s ON a.setorId=s.id
    JOIN Unidade u ON s.unidadeId=u.id
    JOIN User us ON d.responsavelId=us.id WHERE d.id=?`, [docId]);

  if (!doc) return NextResponse.json({ error: "Nao encontrado" }, { status: 404 });

  const itensONA = await query(
    `SELECT oi.codigo,oi.titulo FROM ItemONA oi
     JOIN DocumentoItemONA doi ON oi.id=doi.itemONAId WHERE doi.documentoId=?`,
    [docId]
  );

  const buf = await gerarDocxSemTemplate({
    ...
    itensONA: itensONA.map(i => `${i.codigo} - ${i.titulo}`).join("\n"),
    ano: new Date().getFullYear().toString(),
  });

  const ab = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength);
  const res = new NextResponse(ab, { status: 200 });
  res.headers.set("Content-Type", "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
  res.headers.set("Content-Disposition", `attachment; filename="${doc.codigo}_v${doc.versao}.docx"`);
  return res;
}
