...
export default function ConfirmarLeitura() {
  ...
  useEffect(()=>{
    ...
    fetch("/api/leitura",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token})})
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-10 max-w-md w-full text-center">
        ...
        {state==="ok"&&<><CheckCircle className="w-16 h-16 text-emerald-500 mx-auto mb-4"/><h1 className="text-xl font-bold text-slate-900 mb-2">Leitura confirmada!</h1><p className="text-slate-500 text-sm">Seu registro foi salvo no audit log do GestDoc para fins de conformidade ONA.</p></>}
