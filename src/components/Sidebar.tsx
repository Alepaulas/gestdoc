...
import { LayoutDashboard, FileText, Map, Building2, Settings, LogOut, ChevronRight } from "lucide-react";
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-slate-900 flex flex-col z-50">
      <div className="h-16 flex items-center px-5 border-b border-slate-800 gap-3">
        ...
        <div>
          <p className="text-white font-semibold text-sm leading-none">GestDoc</p>
          <p className="text-slate-500 text-xs mt-0.5">Governança Documental</p>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-slate-800">
        <div className="flex items-center gap-2 px-3 py-2 bg-slate-800 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-slate-500 text-xs truncate">{(session?.user as any)?.role}</p>
          </div>
