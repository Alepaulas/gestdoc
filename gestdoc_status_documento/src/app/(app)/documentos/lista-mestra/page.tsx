...
type Doc = {
  ...
  elaborador: string;
  aprovador: string;
  tempoEmValidacao: number | null;
  tempoEmPadronizacao: number | null;
  obs: string;
  aguardandoAssinatura: string;
};

const AGUARDANDO_ASSINATURA_CONFIG: Record<string, { label: string; bg: string; text: string }> = {
  SIM:           { label: "Sim",           bg: "bg-amber-50", text: "text-amber-700" },
  NAO:           { label: "Não",           bg: "bg-slate-100", text: "text-slate-500" },
  NAO_SE_APLICA: { label: "Não se aplica", bg: "bg-slate-50",  text: "text-slate-400" },
};
...
const STATUS_DOCUMENTO_COLOR: Record<string, string> = {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {["ID","Tipo","Nível","Código","Título","Unidade","Setor","Status Demanda","Status do Documento","Dias","Próx. Revisão","Versão","Publicado em","Elaborador","Aprovador","Tempo Validação","Tempo Padronização","Aguardando Assinatura","Obs"].map(h => (
                ...
                {docs.map(doc => {
                  return (
                    <tr key={doc._linha} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{doc.tipoDocumento}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs text-center whitespace-nowrap">{doc.nivel}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUS_DOCUMENTO_COLOR[doc.statusDocumento] ?? "bg-slate-100 text-slate-500"}`}>
                          {doc.statusDocumento || "—"}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{doc.elaborador}</td>
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{doc.aprovador}</td>
                      <td className="px-4 py-3 text-xs font-mono text-center text-slate-600">
                        {doc.tempoEmValidacao !== null ? `${doc.tempoEmValidacao}d` : "—"}
                      </td>
                      <td className="px-4 py-3 text-xs font-mono text-center text-slate-600">
                        {doc.tempoEmPadronizacao !== null ? `${doc.tempoEmPadronizacao}d` : "—"}
                      </td>
                      <td className="px-4 py-3">
                        {(() => {
                          const aa = AGUARDANDO_ASSINATURA_CONFIG[doc.aguardandoAssinatura] ?? AGUARDANDO_ASSINATURA_CONFIG["NAO_SE_APLICA"];
                          return (
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${aa.bg} ${aa.text}`}>
                              {aa.label}
                            </span>
                          );
                        })()}
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs max-w-xs truncate">{doc.obs}</td>
                    </tr>
                  );
                })}
              </tbody>
