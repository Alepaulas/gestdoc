...
export default function NovoDocumentoPage() {
  ...
  const [form, setForm] = useState({
    ...
    itensONA: "",
    obs: "", aguardandoAssinatura: "NAO_SE_APLICA",
  });
  ...
  async function submit(e: any) {
    ...
    try {
      ...
      const payload = {
        ...
        obs:                  form.obs,
        aguardandoAssinatura: form.aguardandoAssinatura,
      };
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <Section title="Assinatura e Observações">
          <Select label="Aguardando Assinatura" value={form.aguardandoAssinatura} onChange={set("aguardandoAssinatura")}
            options={[
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1.5">Obs</label>
            <textarea value={form.obs} onChange={e => set("obs")(e.target.value)} rows={3}
              placeholder="Observações gerais sobre o documento"
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30"/>
          </div>
        </Section>
