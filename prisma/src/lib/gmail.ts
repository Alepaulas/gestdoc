...
// ── Leitura de e-mails (importação para Solicitações) ───────────
...
function getGmailClientFromTokens(accessToken: string, refreshToken?: string) {
  const oauth2 = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET);
...
function extrairHeader(headers: any[], nome: string): string {
  return headers?.find((h: any) => h.name?.toLowerCase() === nome.toLowerCase())?.value ?? "";
}

/**
 * Lista e-mails recebidos na caixa de entrada do usuário autenticado.
 * `query` segue a sintaxe de busca do Gmail (ex: "is:unread", "newer_than:7d").
 * `maxResults` limita quantos e-mails são buscados (paginação simples).
 */
export async function listarEmailsRecebidos(
  accessToken: string,
  refreshToken: string | undefined,
  query: string = "in:inbox",
  maxResults: number = 25
): Promise<EmailRecebido[]> {
  const gmail = getGmailClientFromTokens(accessToken, refreshToken);
  ...
  for (const id of ids) {
    ...
    const dataStr = extrairHeader(headers, "Date");
    emails.push({
      messageId:    id,
      assunto:      extrairHeader(headers, "Subject") || "(sem assunto)",
      remetente:    extrairHeader(headers, "From"),
      destinatario: extrairHeader(headers, "To"),
      dataEnvio:    dataStr ? new Date(dataStr) : null,
